...
picks_nlp <-
  ...
  rename(bedrock_dep = ".bedrock_dep") |>
  select("gicwellid", "longitude", "latitude", "gr_elev", "bedrock_dep") |>
  mutate(
    data_type = "Borehole",
  ...
  distinct(gicwellid, .keep_all = TRUE)

write_csv(picks, here("outputs/picks.csv"))
write_csv(picks_nlp, here("outputs/picks-nlp.csv"))
```

```{r nlp-prediction-example}
id <- 186147

predict(model_nlp, newdata_feat |> filter(gicwellid == !!id), type = "prob") |> 
...
write_rds(picks_nlp, here("data/processed/picks-nlp.rds"))
```
